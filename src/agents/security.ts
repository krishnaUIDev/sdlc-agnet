...
const SECURITY_PROMPT = `You are the Security Agent. Review the code/design for OWASP vulnerabilities and output a security report.`;

export async function runSecurityAgent() {
  console.log("Starting SECURITY Agent Cycle...");

  try {
    const { data: tasks, error: fetchError } = await supabase
      .from('AgentTask')
      .select('*')
      .eq('agent_id', 'security')
      .eq('status', 'PENDING')
      .order('created_at', { ascending: true })
      .limit(1);

    if (fetchError || !tasks || tasks.length === 0) {
      console.log("No pending tasks for SECURITY.");
      return;
    }

    const task = tasks[0];
    console.log(`[SECURITY] picked up task: ${task.id}`);
    ...
    console.log(`[SECURITY] finished work!`);
    ...
    console.error(`Error running security:`, error);
    try {
      const { data: stuck } = await supabase.from('AgentTask').select('id').eq('agent_id', 'security').eq('status', 'IN_PROGRESS').limit(1);
...
if (require.main === module) runSecurityAgent();
